...
export default function Home() {
  ...
  return (
    <main className="min-h-screen">
      ...
      {/* Floating Navigation */}
      <nav className="fixed top-8 left-1/2 -translate-x-1/2 z-50 transition-all duration-800 ease-in-out px-4 w-full max-w-fit">
        <div className="bg-[var(--card-bg)] backdrop-blur-xl border border-[var(--card-border)] px-4 sm:px-6 py-3 rounded-full flex items-center gap-4 sm:gap-8 shadow-2xl">
          <div className="flex items-center gap-4 sm:gap-8">
            <a href="#home" className="text-[9px] sm:text-[10px] font-mono uppercase tracking-widest text-[var(--fg)] hover:opacity-60 transition-all duration-300">Index</a>
            <a href="#work" className="text-[9px] sm:text-[10px] font-mono uppercase tracking-widest text-[var(--meta)] hover:text-[var(--fg)] transition-all duration-300">Projects</a>
            <a href="#contact" className="text-[9px] sm:text-[10px] font-mono uppercase tracking-widest text-[var(--meta)] hover:text-[var(--fg)] transition-all duration-300">Contact</a>
          </div>
        </div>
      </nav>
          ...
          >
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                ...
                <motion.div variants={itemVariants} className="font-mono text-[10px] uppercase tracking-[0.5em] text-[var(--meta)] opacity-40">
                  System_Log // <span className="text-[var(--fg)] opacity-100">[K]</span> for Commands
                </motion.div>
              </div>
            </div>
            <motion.h1 variants={itemVariants} className="text-5xl sm:text-8xl lg:text-[11rem] font-black tracking-tighter uppercase italic leading-[0.8] sm:leading-[0.7] text-[var(--fg)]">
              Seonay<br />
              <span className="text-black dark:text-transparent outline-text dark:outline-text opacity-40">Studio</span>
            </motion.h1>
          </motion.div>
